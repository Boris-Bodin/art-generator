"""Tests du compositing alpha, de l'encre soustractive, des fonds et du cadrage."""
...
def test_light_generator_backgrounds_keep_color_in_gradients():
    colorful = False
    for seed in range(80):
        kind, params = _light_background(RNG(seed))
        if kind == "black":
            continue
        key_a, key_b = ("top", "bottom") if kind == "gradient" else ("inner", "outer")
        a = np.asarray(params[key_a])
        b = np.asarray(params[key_b])
        assert a.max() >= 0.12
        assert b.max() > 0.02
        if abs(float(a.argmax()) - float(b.argmax())) > 0 or np.ptp(a) > 0.03:
            colorful = True
    assert colorful
